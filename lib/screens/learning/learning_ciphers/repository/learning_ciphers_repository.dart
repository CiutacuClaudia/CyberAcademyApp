...
class LearningCiphersRepository {
  final InterceptedClient _client = InterceptedClient.build(
    interceptors: [LoggingInterceptor(), AuthInterceptor()],
  );

  Future<ApiResult<List<LearningCipher>>> fetchCiphers() async {
    final url = Uri.parse('${ApiConfig.baseUrl}/cyberacademy/learning/ciphers');
    ...
    if (response.statusCode == ApiStatusCode.ok) {
      ...
      final ciphers =
          list
              .map((e) => LearningCipher.fromJson(e as Map<String, dynamic>))
              .toList();
